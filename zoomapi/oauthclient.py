...
class OAuthZoomClient(ZoomClient):
    ...
    def __init__(
        ...
        """
        super(OAuthZoomClient, self).__init__(
            api_key=client_id, api_secret=client_secret, timeout=timeout
        )
        ...
        self.config["token"] = util.get_oauth_token(
            client_id, client_secret, port, redirect_url, browser_path
        )

        self.components[
            "chat_channels"
        ] = components.chat_channels.ChatChannelsComponentV2
        self.components[
            "chat_messages"
    ...
    @property
    def chat_messages(self) -> components.chat_messages.ChatMessagesComponentV2:
